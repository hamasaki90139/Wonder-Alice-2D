...
void DrawResult(void)
{
	...
	case 1:
		// テクスチャの設定
		pDevice->SetTexture(0, g_pD3DTextureResultmenu1);
		{
			// 頂点フォーマットの設定
			pDevice->SetFVF(FVF_VERTEX_2D);

			// ポリゴンの描画
			pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, NUM_POLYGON, &g_vertexWkResultmenu1, sizeof(VERTEX_2D));
		}

		// テクスチャの設定
		pDevice->SetTexture(0, g_pD3DTextureResultmenu2);
		{
			// 頂点フォーマットの設定
			pDevice->SetFVF(FVF_VERTEX_2D);

			// ポリゴンの描画
			pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, NUM_POLYGON, &g_vertexWkResultmenu2, sizeof(VERTEX_2D));
		}
	}
}
